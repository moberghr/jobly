...
const WrapperComponent: React.FC<WrapperComponentProps> = ({ WrappedComponent, additionalProp, children }) => {
...
const Table = ({ data, columnNames, specialColumns, specialColumnComponents }: ITableProps) => {
			...
			<div className="table-footer">
				{data.data.length > 0 && (
					<>
						...
						<p>
							{itemsPerPage * (currentPage - 1)}-{itemsPerPage * (currentPage - 1) + data.data.length} of{" "}
							<b>{data.totalCount}</b>
						</p>

						<Pagination size="sm">
							<Pagination.First disabled={currentPage === 1} onClick={() => handlePaginationChange(1)} />
							<Pagination.Prev
								disabled={currentPage === 1}
								onClick={() => handlePaginationChange(currentPage - 1)}
							/>
							<Pagination.Item active={currentPage === 1} onClick={() => handlePaginationChange(1)}>
								{1}
							</Pagination.Item>
				...
				{!data.data || (data.data.length === 0 && <p className="no-data">There is no data.</p>)}
			</div>
		</>
	);
};
