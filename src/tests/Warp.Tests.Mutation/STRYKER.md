...
Mutation testing for Warp using a dedicated SQLite-only test project (`Warp.Tests.Mutation`). No Docker required — runs 293 tests in ~10 seconds.

## How to run

```bash
cd src/tests/Warp.Tests.Mutation
...
- **`Warp.Tests.Mutation/`** — Separate test project with only SQLite fixture. No Testcontainers, no Docker.
- **`stryker-config.json`** — Core config. Mutates `Warp.Core`, excludes entities/DTOs/interfaces.
- **`stryker-config.worker.json`** — Worker config. Mutates `Warp.Worker`, lower thresholds.
- **`stryker.slnx`** — Mini solution for Stryker that excludes `Warp.Tests` (prevents Docker container startup).
...
Worker score is low because `WarpWorkerService` (the execution loop) and `ServerTaskBase` (polling infrastructure) are hard to unit test — they're covered by integration tests on PostgreSQL.

## Adding tests for new code

When adding a new test file to `Warp.Tests`:
1. If it has `[Collection<PostgreSqlCollection>]`, create a corresponding `_Sqlite.cs` file in `Warp.Tests.Mutation/Unit/`:

```csharp
using Warp.Tests.Mutation.Fixtures;
using Warp.Tests.Unit;
...
### Stryker picks up Warp.Tests (Docker containers)
The `stryker.slnx` mini solution excludes `Warp.Tests`. Make sure the config has `"solution": "stryker.slnx"`.
