    ...
    where TContext : DbContext
{
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                while (!stoppingToken.IsCancellationRequested)
                {
                    ...
                    var workDone = await RunOrchestration(context, stoppingToken);
                    if (!workDone)
                    {
                        break;
                    }
    ...
    /// </summary>
    public static async Task<bool> RunOrchestration<TCtx>(TCtx context, CancellationToken ct)
        where TCtx : DbContext
    {
        // Clear change tracker between steps to avoid stale tracked entities
        var finalized = await FinalizeParents(context, ct);
    ...
    /// </summary>
    private static async Task<int> FinalizeParents<TCtx>(TCtx context, CancellationToken ct)
        where TCtx : DbContext
    {
        // Find parents where:
        ...
        foreach (var parent in readyParents)
        {
            ...
            parent.ExpireAt = DateTime.UtcNow.AddDays(1);
        }
