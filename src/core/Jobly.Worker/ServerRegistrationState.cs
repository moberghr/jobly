...
/// guaranteed by DI registration order in <c>AddJoblyWorker</c>.
/// <para>
    ...
    public void Set(IReadOnlyList<GroupRegistration> groups)
    {
        if (_groups.Count > 0)
        {
            throw new InvalidOperationException(
                "ServerRegistrationState.Set was called more than once. This singleton is populated once by JoblyServerRegistration at host startup and read by the worker host services afterwards — a second Set indicates a partial host recycle with live workers holding stale IDs.");
        }
