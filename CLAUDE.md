...
Warp is a distributed job processing and message queue library for .NET 10. Five patterns:
...
- **Sagas** (`Saga` + `ISagaHandler<TSaga, TMessage>`) — Long-lived, correlated state across multiple message arrivals. Opt-in via `opt.AddSagas()`. See §8.9.
...
- Opt-in addons on the builder: `opt.AddRetry()`, `opt.AddConcurrency()` (Mutex + Semaphore), `opt.AddCircuitBreaker()`, `opt.AddNoRestart()`, `opt.UseDatabasePush()`
...
- **§2.1** Execution-orchestration entities (jobs, messages, batches) share one table, one state machine, one parent-child chain. Use `Kind` discriminator and `ParentJobId` chain. Correlated-state entities that don't participate in this lifecycle — e.g. `SagaState` — live in their own tables. See §8.9.
...
- **§8.9** Sagas are an opt-in addon (`opt.AddSagas()` on the builder) in `Warp.Core.Sagas`. Users subclass `Saga` (or `Saga<TKey>` for typed correlation access) for state, annotate the correlation property on `IMessage` types with `[Correlate]`, and mark saga-initiating messages with `[StartsSaga]`. `[Correlate]` accepts `string`, `Guid`, `int`, or `long` — `SagaCorrelationKeyConverter` canonicalizes to a single string column (`Guid` → `ToString("N")`, integers → invariant-culture decimal). `Saga<TKey>.Key` is the typed projection of the persisted string. Storage stays one `varchar(200)` column on the `SagaState` entity (the user-facing `Saga` base class is in `Warp.Core.Sagas`; the storage row type is `Warp.Core.Data.Entities.SagaState`, named to avoid the obvious collision), with a strict unique index on `(Type, CorrelationKey)`. `Saga.Id`, `Saga.CorrelationKey`, `Saga.IsCompleted`, and `Saga<TKey>.Key` are all `[JsonIgnore]`'d — `StateJson` contains only the user's subclass-defined state. **Sagas never expire automatically.** They live until `MarkCompleted()` (handler-initiated) or `ISagaCommandService.ForceComplete` (operator-initiated via dashboard). No TTL. Users wanting auto-expiry schedule a self-cleanup `ITimeoutMessage`. **Dashboard:** opt-in pages at `/warp/sagas` and `/warp/sagas/{id}` with hide-on-404 nav. Activity log uses a separate `SagaJobLink` extension table (`(SagaId, JobId, CreatedAt)`) the proxy writes on every invocation, cleaned up via change-tracker `RemoveRange` so deletes commit atomically with the saga row in a single `SaveChanges` transaction. DB-level cascade FK is the belt-and-braces backstop. Activity log query caps at 200 most-recent invocations to avoid SQL Server's ~2100 parameter limit on `Contains()`. Saga timeouts use `ITimeoutMessage` (in `Warp.Core.Handlers`) — implement it on a message class with a `TimeSpan Delay`; `Publisher.Publish` auto-schedules the message at `now + Delay` (state `Scheduled`, activated by `ScheduledJobActivation`). When an `ITimeoutMessage` arrives for a missing saga (already completed and deleted), `SagaHandlerProxy` silently sets `Outcome = Deleted` rather than `Failed` — pending timeouts don't need explicit cancellation on saga completion. Handler classes implement `ISagaHandler<TSaga, TMessage>` for each message they handle and are registered via `services.AddSagaHandler<THandler>()` — reflection wires a `SagaHandlerProxy<TSaga, TMessage>` as the `IMessageHandler<TMessage>` for each closed interface. The proxy: extracts the correlation key, acquires a mutex on `warp:saga:{TSaga.FullName}:{key}` via `IWarpSemaphoreProvider`, loads the saga (or creates if `[StartsSaga]`), invokes the handler, and persists state (insert / update / **delete-on-completion**). Completion is signalled via the public `Saga.MarkCompleted()` method — the row is removed in the same `SaveChanges` so the correlation key is immediately reusable. Missing-saga messages hit the handler's `NotFoundAsync` override (default: pre-set `Failed` outcome). Mutex contention requeues the message with `ClearHandlerType = false` so the next attempt re-enters the proxy. Optimistic concurrency on `SagaState.Version` is defense-in-depth — `DbUpdateConcurrencyException` also requeues. No `[StartsSaga]` granularity per-method — apply per message class. Sagas attach only to `IMessage`, not `IJob`. Telemetry counters `warp.sagas.started/completed/requeued`. See `website/docs/features/sagas.md`.
