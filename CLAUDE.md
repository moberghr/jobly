...
- **§2.8** Future-dated jobs land in `State.Scheduled`; `ScheduledJobActivationTask` flips them to `Enqueued` when `ScheduleTime <= now`. Worker fetch queries always check `CurrentState == Enqueued` (with a defensive `ScheduleTime <= now` predicate for pre-upgrade legacy rows). Adding new query sites that filter by `Enqueued` without the time predicate is a latent bug on upgraded deployments.
